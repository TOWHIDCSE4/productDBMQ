...
export default class ProductController extends BaseController {
  ...
  async delete() {
    ...
    let products = await this.Model.query().whereIn('id', params.ids);
    if (products.length !== params.ids.length) throw new ApiException(6000, "product doesn't exist!")

    let checkUser = await this.UserModel.query().whereIn('productId', params.ids)
    if (!_.isEmpty(checkUser)) throw new ApiException(6004, "Product contains user cannot be deleted!")
